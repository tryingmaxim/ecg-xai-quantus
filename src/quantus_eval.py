# src/quantus_eval.py
#   python -m src.quantus_eval --model resnet18 --method gradcam --limit 144
...
def make_gradcam_explainer(model):
    ...
    class GradCAM:
        def __init__(self, model, target):
            ...
            def fwd_hook(_, __, out): self.acts = out.detach()
            def bwd_hook(_, grad_in, grad_out): self.grads = grad_out[0].detach()
...
def main():
    ...
    ds, loader = build_loader(args.data_dir, args.batch, args.num_workers, limit=args.limit)
    ...
    H = torch.cat(Hs).numpy()[:,0,:,:]   # (N,H,W)

    # ------------- QUANTUS METRICS (fixed!) --------------------
    from quantus import (

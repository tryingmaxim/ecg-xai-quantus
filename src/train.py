...
def train_one_model(model_name: str):
    ...
    if ds_classes is None:
        print(
            f"[WARN] Konnte Klassen aus '{train_dir}' nicht ermitteln. "
    ...
    if getattr(configs, "USE_COSINE_SCHEDULER", False):
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            optimizer, T_max=configs.EPOCHS
        )
    ...
    for epoch in range(1, configs.EPOCHS + 1):
        ...
        dt = time.time() - t0
        print(
            f"[{model_name}] Epoch {epoch:02d}/{configs.EPOCHS} | "
        ...
        if val_acc > best_acc:
            ...
            print(
                f"[{model_name}] [CHECK] Neues bestes Modell gespeichert: {ckpt_path}"
            )
        else:
            no_improve += 1
